...
def write_rumors(info: list) -> None:
    ...
    for rumor in info:
        io.write_int(io.str_byte_len(rumor["name"]), 4)
        io.write_str(    rumor["name"])
        io.write_int(io.str_byte_len(rumor["text"]), 4)
...
def write_events(info: list, is_town: bool = False) -> None:
    ...
    for event in info:
        io.write_int(io.str_byte_len(event["name"]), 4)
        io.write_str(    event["name"])
        io.write_int(io.str_byte_len(event["message"]), 4)
