...
import src.handler_01_general as h1  # For map format detection
...
# Helper functions for format-aware artifact and creature ID reading/writing
# RoE uses 1 byte for artifact/creature IDs, AB/SoD/HotA use 2 bytes

def read_artifact_id() -> int:
    """Read artifact ID - 1 byte for RoE, 2 bytes for AB/SoD/HotA"""
    map_format = h1.current_map_format
    if map_format == h1.MapFormat.RoE:
        artifact_id = io.read_int(1)
        # RoE: 0xFF means no artifact, normalize to 0xFFFF
        if artifact_id == 0xFF:
            return 0xFFFF
        return artifact_id
    return io.read_int(2)

def write_artifact_id(artifact_id: int) -> None:
    """Write artifact ID - 1 byte for RoE, 2 bytes for AB/SoD/HotA"""
    map_format = h1.current_map_format
    if map_format == h1.MapFormat.RoE:
        ...
        io.write_int(artifact_id, 2)

def read_creature_id() -> int:
    """Read creature ID - 1 byte for RoE, 2 bytes for AB/SoD/HotA"""
    map_format = h1.current_map_format
    if map_format == h1.MapFormat.RoE:
        creature_id = io.read_int(1)
        # RoE: 0xFF means no creature, normalize to 0xFFFF
        if creature_id == 0xFF:
            return 0xFFFF
        return creature_id
    return io.read_int(2)

def write_creature_id(creature_id: int) -> None:
    """Write creature ID - 1 byte for RoE, 2 bytes for AB/SoD/HotA"""
    map_format = h1.current_map_format
    if map_format == h1.MapFormat.RoE:
        if creature_id == 0xFFFF:
            creature_id = 0xFF
        io.write_int(creature_id, 1)
    else:
        io.write_int(creature_id, 2)
...
def write_object_defs(info: list) -> None:
    ...
    for obj in info:
        io.write_int(io.str_byte_len(obj["sprite"]), 4)
...
def get_subtype(obj_type: int, i: int) -> int:
    # Python 3.9 compatible: converted from match statement
    if obj_type == od.ID.Artifact:                  return ad.ID(i)
    elif obj_type == od.ID.Border_Guard:            return od.Border_Color(i)
    elif obj_type == od.ID.Keymasters_Tent:         return od.Border_Color(i)
    elif obj_type == od.ID.Cartographer:            return od.Cartographer(i)
    elif obj_type == od.ID.Creature_Bank:           return od.Creature_Bank(i)
    elif obj_type == od.ID.Creature_Generator_1:    return od.Dwelling(i)
    elif obj_type == od.ID.Creature_Generator_4:    return od.Dwelling_Multi(i)
    elif obj_type == od.ID.Hero:                    return hd.Classes(i)
    elif obj_type == od.ID.Hill_Fort:               return od.Hill_Fort(i)
    elif obj_type == od.ID.Monolith_One_Way_Entrance: return od.One_Way_Monolith(i)
    elif obj_type == od.ID.Monolith_One_Way_Exit:   return od.One_Way_Monolith(i)
    elif obj_type == od.ID.Two_Way_Monolith:        return od.Two_Way_Monolith(i)
    elif obj_type == od.ID.Mine:                    return od.Resource(i)
    elif obj_type == od.ID.Monster:                 return cd.ID(i)
    elif obj_type == od.ID.Resource:                return od.Resource(i)
    elif obj_type == od.ID.Town:                    return od.Town(i)
    elif obj_type == od.ID.HotA_Decoration_1:       return od.HotA_Decoration_1(i)
    elif obj_type == od.ID.HotA_Decoration_2:       return od.HotA_Decoration_2(i)
    elif obj_type == od.ID.HotA_Ground:             return od.HotA_Ground(i)
    elif obj_type == od.ID.HotA_Warehouse:          return od.Resource(i)
    elif obj_type == od.ID.HotA_Visitable_1:        return od.HotA_Visitable_1(i)
    elif obj_type == od.ID.HotA_Collectible:        return od.HotA_Collectible(i)
    elif obj_type == od.ID.HotA_Visitable_2:        return od.HotA_Visitable_2(i)
    elif obj_type == od.ID.Border_Gate:             return od.Border_Color(i)
    return i
...
def parse_object_data(object_defs: list) -> list:
    ...
    for _ in range(io.read_int(4)): # Amount of objects
        ...
        # Python 3.9 compatible: converted from match statement
        obj_type = obj["type"]
        if obj_type == od.ID.Pandoras_Box:       obj = parse_pandoras_box(obj)
        elif obj_type == od.ID.Black_Market:     obj = parse_black_market(obj)
        elif obj_type == od.ID.Campfire:         obj = parse_campfire(obj)
        elif obj_type == od.ID.Corpse:           obj = parse_corpse(obj)
        elif obj_type == od.ID.Event:            obj = parse_event(obj)
        elif obj_type == od.ID.Flotsam:          obj = parse_flotsam(obj)
        elif obj_type == od.ID.Lean_To:          obj = parse_lean_to(obj)
        elif obj_type == od.ID.Pyramid:          obj = parse_pyramid(obj)
        elif obj_type == od.ID.Scholar:          obj = parse_scholar(obj)
        elif obj_type == od.ID.Sea_Chest:        obj = parse_sea_chest(obj)
        elif obj_type == od.ID.Seers_Hut:        obj = parse_seers_hut(obj)
        elif obj_type == od.ID.Shipwreck_Survivor: obj = parse_shipwreck_survivor(obj)
        elif obj_type == od.ID.Treasure_Chest:   obj = parse_treasure_chest(obj)
        elif obj_type == od.ID.Tree_of_Knowledge: obj = parse_tree_of_knowledge(obj)
        elif obj_type == od.ID.University:       obj = parse_university(obj)
        elif obj_type == od.ID.Wagon:            obj = parse_wagon(obj)
        elif obj_type == od.ID.Warriors_Tomb:    obj = parse_warriors_tomb(obj)
        elif obj_type == od.ID.Random_Dwelling:  obj = parse_dwelling(obj)
        elif obj_type == od.ID.Random_Dwelling_Leveled: obj = parse_leveled(obj)
        elif obj_type == od.ID.Random_Dwelling_Faction: obj = parse_faction(obj)
        elif obj_type == od.ID.Quest_Guard:      obj["quest"] = parse_quest()
        elif obj_type == od.ID.Grail:            obj["radius"] = io.read_int(4)
        elif obj_type == od.ID.Witch_Hut:        obj["skills"] = io.read_bits(4)
        elif obj_type == od.ID.HotA_Collectible: obj = parse_hota_collectible(obj)
        elif obj_type == od.ID.Abandoned_Mine:   obj = parse_abandoned_mine(obj)
        elif obj_type == od.ID.Mine:
            if obj["subtype"] == od.Resource.Abandoned:
                obj = parse_abandoned_mine(obj)
            else: obj["owner"] = io.read_int(4)
        elif obj_type == od.ID.HotA_Visitable_2:
            if obj["subtype"] == 0: # HotA Seafaring_Academy
                obj = parse_university(obj)
        elif obj_type == od.ID.Border_Gate:
            ...
            if obj["subtype"] == 1000: # HotA Quest Gate
                ...
                obj = parse_grave(obj)
        elif obj_type == od.ID.Town:             obj = parse_town(obj)
        elif obj_type == od.ID.Random_Town:      obj = parse_town(obj, random=True)
        elif obj_type in (od.ID.Resource, od.ID.Random_Resource):
            obj = parse_resource(obj)
        elif obj_type in (od.ID.Hero, od.ID.Prison, od.ID.Random_Hero):
            obj = parse_hero(obj)
        elif obj_type in (od.ID.Monster, od.ID.Random_Monster,
        ...
        elif obj_type in (od.ID.Artifact, od.ID.Random_Artifact,
        ...
        elif obj_type in (od.ID.Ocean_Bottle, od.ID.Sign):
            obj["message"] = io.read_str(io.read_int(4))
            io.seek(4)
        elif obj_type in (od.ID.Creature_Generator_1, od.ID.Lighthouse,
                          od.ID.Creature_Generator_4, od.ID.Shipyard):
            obj["owner"] = io.read_int(4)
        elif obj_type in (od.ID.Garrison, od.ID.Garrison_Vertical):
            obj = parse_garrison(obj)
        elif obj_type in (od.ID.Shrine_of_Magic_Incantation,
        ...
        elif obj_type == od.ID.Spell_Scroll:
            obj = parse_spell_scroll(obj)
        elif obj_type == od.ID.Hero_Placeholder:
            obj = parse_hero_placeholder(obj)
        elif obj_type in (od.ID.Creature_Bank, od.ID.Derelict_Ship,
...
def write_object_data(info: list) -> None:
    ...
    for obj in info:
        ...
        # Python 3.9 compatible: converted from match statement
        obj_type = obj["type"]
        if obj_type == od.ID.Pandoras_Box:       write_pandoras_box(obj)
        elif obj_type == od.ID.Black_Market:     write_black_market(obj)
        elif obj_type == od.ID.Campfire:         write_campfire(obj)
        elif obj_type == od.ID.Corpse:           write_corpse(obj)
        elif obj_type == od.ID.Event:            write_event(obj)
        elif obj_type == od.ID.Flotsam:          write_flotsam(obj)
        elif obj_type == od.ID.Lean_To:          write_lean_to(obj)
        elif obj_type == od.ID.Pyramid:          write_pyramid(obj)
        elif obj_type == od.ID.Scholar:          write_scholar(obj)
        elif obj_type == od.ID.Sea_Chest:        write_sea_chest(obj)
        elif obj_type == od.ID.Seers_Hut:        write_seers_hut(obj)
        elif obj_type == od.ID.Shipwreck_Survivor: write_shipwreck_survivor(obj)
        elif obj_type == od.ID.Treasure_Chest:   write_treasure_chest(obj)
        elif obj_type == od.ID.Tree_of_Knowledge: write_tree_of_knowledge(obj)
        elif obj_type == od.ID.University:       write_university(obj)
        elif obj_type == od.ID.Wagon:            write_wagon(obj)
        elif obj_type == od.ID.Warriors_Tomb:    write_warriors_tomb(obj)
        elif obj_type == od.ID.Random_Dwelling:  write_dwelling(obj)
        elif obj_type == od.ID.Random_Dwelling_Leveled: write_leveled(obj)
        elif obj_type == od.ID.Random_Dwelling_Faction: write_faction(obj)
        elif obj_type == od.ID.Quest_Guard:      write_quest(obj["quest"])
        elif obj_type == od.ID.Grail:            io.write_int(obj["radius"], 4)
        elif obj_type == od.ID.Witch_Hut:        io.write_bits(obj["skills"])
        elif obj_type == od.ID.HotA_Collectible: write_hota_collectible(obj)
        elif obj_type == od.ID.Abandoned_Mine:   write_abandoned_mine(obj)
        elif obj_type == od.ID.Mine:
            if obj["subtype"] == od.Resource.Abandoned:
            ...
            if obj["subtype"] == 1000: # HotA Quest Gate
                ...
                write_grave(obj)
        elif obj_type == od.ID.Town:             write_town(obj)
        elif obj_type == od.ID.Random_Town:      write_town(obj, random=True)
        elif obj_type in (od.ID.Resource, od.ID.Random_Resource):
            write_resource(obj)
        elif obj_type in (od.ID.Hero, od.ID.Prison, od.ID.Random_Hero):
            write_hero(obj)
            ...
            io.write_int(io.str_byte_len(obj["message"]), 4)
...
def parse_hota_collectible(obj: dict) -> dict:
    # Python 3.9 compatible: converted from match statement
    subtype = obj["subtype"]
    if subtype == od.HotA_Collectible.Ancient_Lamp:   obj = parse_ancient_lamp(obj)
    elif subtype == od.HotA_Collectible.Sea_Barrel:   obj = parse_sea_barrel(obj)
    elif subtype == od.HotA_Collectible.Jetsam:       obj = parse_flotsam(obj)
    elif subtype == od.HotA_Collectible.Vial_of_Mana: obj = parse_vial_of_mana(obj)
    return obj

def write_hota_collectible(obj: dict) -> None:
    # Python 3.9 compatible: converted from match statement
    subtype = obj["subtype"]
    if subtype == od.HotA_Collectible.Ancient_Lamp:   write_ancient_lamp(obj)
    elif subtype == od.HotA_Collectible.Sea_Barrel:   write_sea_barrel(obj)
    elif subtype == od.HotA_Collectible.Jetsam:       write_flotsam(obj)
    elif subtype == od.HotA_Collectible.Vial_of_Mana: write_vial_of_mana(obj)

def parse_creatures(amount: int = 7) -> list:
    info = []
    for _ in range(amount):
        creature = {}
        creature["id"] = cd.ID(read_creature_id())
...
def parse_hero(obj: dict) -> dict:
    ...
    # HotA-specific hero fields (1.7.0+)
    map_format = h1.current_map_format
...
def parse_monster(obj: dict) -> dict:
    ...
    if io.read_int(1):
        ...
        obj["artifact"] = ad.ID(read_artifact_id())
...
def write_monster(obj: dict) -> None:
    ...
    if "message" in obj:
        ...
        for res in obj["resources"]:
            io.write_int(res, 4)
        write_artifact_id(obj["artifact"])
    else: io.write_int(0, 1)
...
def write_town(obj: dict, random: bool = False) -> None:
    ...
    if "name" in obj:
        io.write_int(1, 1)
        io.write_int(io.str_byte_len(obj["name"]), 4)
...
def parse_scholar(obj: dict) -> dict:
    ...
    if reward_type == 255:   io.seek(1) # Random
    elif reward_type == 0:   obj["reward_value"] = skd.Primary(io.read_int(1))
    elif reward_type == 1:   obj["reward_value"] = skd.Secondary(io.read_int(1))
    elif reward_type == 2:   obj["reward_value"] = spd.ID(io.read_int(1))
...
def parse_quest() -> dict:
    ...
    if quest_type == Quest.NONE:
        return quest
    elif quest_type == Quest.ACHIEVE_EXPERIENCE_LEVEL:
        quest["value"] = io.read_int(4)
    elif quest_type == Quest.ACHIEVE_PRIMARY_SKILL_LEVEL:
        skills = []
        ...
        skills.append(io.read_int(1))
        quest["value"] = skills
    elif quest_type in (Quest.DEFEAT_SPECIFIC_HERO, Quest.DEFEAT_SPECIFIC_MONSTER):
    ...
    elif quest_type == Quest.RETURN_WITH_ARTIFACTS:
        quest["value"] = []
        map_format = h1.current_map_format
        for _ in range(io.read_int(1)):
            if map_format == h1.MapFormat.RoE:
                ...
                if artifact_id == 0xFF:
                    artifact_id = 0xFFFF
                quest["value"].append([ad.ID(artifact_id), 0])
            else:
                quest["value"].append([
                    ad.ID(io.read_int(2)),
                    spd.ID(io.read_int(2))
                ])
    elif quest_type == Quest.RETURN_WITH_CREATURES:
        quest["value"] = parse_creatures(amount=io.read_int(1))
    elif quest_type == Quest.RETURN_WITH_RESOURCES:
        quest["value"] = []
        for _ in range(7):
            quest["value"].append(io.read_int(4))
    elif quest_type == Quest.BE_SPECIFIC_HERO:
        quest["value"] = hd.ID(io.read_int(1))
    elif quest_type == Quest.BELONG_TO_SPECIFIC_PLAYER:
        quest["value"] = io.read_int(1)
    elif quest_type == Quest.HOTA_QUEST:
...
def write_quest(info: dict) -> None:
    ...
    elif quest_type in (Quest.BE_SPECIFIC_HERO, Quest.BELONG_TO_SPECIFIC_PLAYER):
    ...
    io.write_int(io.str_byte_len(info["proposal_message"]), 4)
    io.write_str(    info["proposal_message"])
    io.write_int(io.str_byte_len(info["progress_message"]), 4)
    io.write_str(    info["progress_message"])
    io.write_int(io.str_byte_len(info["completion_message"]), 4)
...
def parse_reward() -> dict:
    ...
    if reward_type in (Reward.EXPERIENCE, Reward.SPELL_POINTS):
        reward["value"] = io.read_int(4)
    elif reward_type in (Reward.MORALE, Reward.LUCK):
        reward["value"] = io.read_int(1)
    elif reward_type == Reward.RESOURCE:
        reward["value"] = []
        reward["value"].append(od.Resource(io.read_int(1)))
        reward["value"].append(io.read_int(4))
    elif reward_type == Reward.PRIMARY_SKILL:
        reward["value"] = []
        reward["value"].append(skd.Primary(io.read_int(1)))
        reward["value"].append(io.read_int(1))
    elif reward_type == Reward.SECONDARY_SKILL:
        reward["value"] = []
        reward["value"].append(skd.Secondary(io.read_int(1)))
        reward["value"].append(io.read_int(1))
    elif reward_type == Reward.ARTIFACT:
        map_format = h1.current_map_format
        if map_format == h1.MapFormat.RoE:
            artifact_id = io.read_int(1)
            if artifact_id == 0xFF:
                artifact_id = 0xFFFF
            reward["value"] = [ad.ID(artifact_id), 0]
        else:
            reward["value"] = [
                ad.ID(io.read_int(2)),
                spd.ID(io.read_int(2))
            ]
    elif reward_type == Reward.SPELL:
        reward["value"] = spd.ID(io.read_int(1))
    elif reward_type == Reward.CREATURES:
...
def write_reward(info: dict) -> None:
    ...
    if reward_type in (Reward.EXPERIENCE, Reward.SPELL_POINTS):
        io.write_int(info["value"], 4)
    elif reward_type in (Reward.MORALE, Reward.LUCK, Reward.SPELL):
        io.write_int(info["value"], 1)
    elif reward_type == Reward.RESOURCE:
        io.write_int(info["value"][0], 1)
        io.write_int(info["value"][1], 4)
    elif reward_type in (Reward.PRIMARY_SKILL, Reward.SECONDARY_SKILL):
        io.write_int(info["value"][0], 1)
        io.write_int(info["value"][1], 1)
    elif reward_type == Reward.ARTIFACT:
        map_format = h1.current_map_format
        if map_format == h1.MapFormat.RoE:
            artifact_id = info["value"][0]
            if artifact_id == 0xFFFF:
                artifact_id = 0xFF
            io.write_int(artifact_id, 1)
        else:
